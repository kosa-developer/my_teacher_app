...
//                $('#file_div' + id).html('<img style="width:100%;"id="blah' + id + '" src="staff_image/person.jpg" alt="your image" align="left"/><br/><br/><p style="color:blue">Upload a clear and professional photo .</p><input type="file" class="form-control" id="i_file" name="std_photo" accept="image/*" onchange="readURL(this,"' + id + '");" />');
    ...
    function Check() {
        ...
        for (i = 0; i < checkBoxes.length; i++) {
            ...
            var res = staff_id.substr(7);

            clicked_policy(res, staff_name)
        }
    ...
    function clicked_policy(staff_id, staff_name) {
        ...
        if (document.getElementById('policy_' + staff_id).checked) {
            document.getElementById('id_data_policy').insertAdjacentHTML('beforeend', '<div id="' + row_ids + '_policy" class="col-lg-12 col-md-12 col-sm-12"><div class="col-lg-3 col-md-3 col-sm-3"><label>' + staff_name + '</label>\n\
<input type="hidden" value="' + staff_name + '" class="form-control" name="staff_name_[]"/>\n\
<input type="hidden" value="' + staff_id + '" class="form-control" name="staff_id_[]"/></div>\n\
                  <div  class="col-lg-8 col-md-8 col-sm-8"><input type="email" name="email_[]" class="form-control pull-right" required> <input type="hidden" value="' + staff_id + '<?php echo generatePasswordz(); ?>" name="code_[]" class="form-control pull-right" required></div>\n\
<div class="col-lg-1 col-md-1 col-sm-1"><button type="button" value="' + row_ids + '_policy" class="fa fa-trash-o btn btn-danger btn-xs" onclick="delete_item1(this.value);"></button></div></div>');
